  ...
  try {
    ...
    await getPool().query(
      `INSERT INTO registrations (id, user_id, program_type, program_id, program_name, status, details)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [id, user.id, programType, programId, programName, 'submitted', JSON.stringify(details)],
    );
    ...
    return res.status(201).json({ message: 'Registration submitted successfully.', registration });
  } catch (err) {
    return handleApiError(res, err, 'Error submitting registration.');
  }
}
